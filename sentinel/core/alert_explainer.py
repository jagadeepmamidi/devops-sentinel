...
class AlertExplainer:
    ...
    def _generate_summary(self, incident: dict, service: dict | None) -> str:
    ...
    def _generate_reasons(
        self, incident: dict, service: dict | None, baseline: dict | None
    ) -> list[str]:
    ...
    def _generate_context(
        self, incident: dict, deployment: dict | None, dependencies: list[dict]
    ) -> list[str]:
        ...
        if deployment:
            ...
            if deployed_at:
                try:
                    ...
                    minutes_ago = (
                        datetime.now(timezone.utc) - deploy_time.replace(tzinfo=None)
                    ).seconds // 60
    ...
    def _generate_actions(self, incident: dict, similar: dict | None) -> list[str]:
        """Generate suggested next actions"""
        actions = []

        # If similar incident found with resolution
        if similar and similar.get("resolution_notes"):
            actions.append(f"SUGGESTED: {str(similar.get('resolution_notes', ''))[:200]}")
    ...
    async def _get_incident(self, incident_id: str) -> dict | None:
        """Get incident details"""
        result = await self.supabase.table("incidents").select("*").eq("id", incident_id).execute()

        return result.data[0] if result.data else None

    async def _get_service(self, service_id: str) -> dict | None:
        """Get service details"""
        result = await self.supabase.table("services").select("*").eq("id", service_id).execute()

        return result.data[0] if result.data else None

    async def _get_baseline(self, service_id: str) -> dict | None:
        """Get service baseline metrics"""
        result = (
            await self.supabase.table("baselines")
            .select("*")
    ...
    async def _get_recent_deployment(self, service_id: str) -> dict | None:
        ...
        result = (
            await self.supabase.table("deployments")
            .select("*")
            .eq("service_id", service_id)
            .gte("deployed_at", one_hour_ago)
    ...
    async def _find_similar_incident(self, incident: dict) -> dict | None:
        ...
        result = (
            ...
            .eq("failure_type", incident.get("failure_type", ""))
            .eq("status", "resolved")
