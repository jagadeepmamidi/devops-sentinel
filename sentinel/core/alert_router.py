...
class AlertRouter:
    ...
    async def route_incident(self, incident: dict, immediate: bool = True) -> dict:
        """
    ...
    async def escalate_incident(
        self, incident_id: str, current_assignee: str, reason: str = "No acknowledgment"
    ) -> dict | None:
        """
    ...
    async def _get_on_call_user(
        self, severity: str, service_id: str, priority: int = 1
    ) -> dict | None:
        """
    ...
    async def _send_notifications(self, incident: dict, routing: dict):
        """
    ...
    async def _send_slack_notification(self, incident: dict, routing: dict):
        ...
        print(
            f"📢 Slack: {routing['severity']} incident {incident.get('id')} → {routing['assigned_name']}"
        )
