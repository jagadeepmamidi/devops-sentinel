...
class TeamAssignmentSystem:
    ...
    async def assign_incident(
        ...
        incident = (
            await self.supabase.table("incidents")
            .select("team_id, severity, service_id")
            .eq("id", incident_id)
            .execute()
        )
        ...
        await self.supabase.table("incident_assignments").insert(assignment).execute()

        # Update incident
        await (
            self.supabase.table("incidents")
            .update({"assigned_to": assigned_to, "status": "assigned"})
            .eq("id", incident_id)
            .execute()
        )
    ...
    async def _auto_assign(self, team_id: str, severity: str) -> str | None:
        ...
        if severity in ["P0", "P1"]:
            # Get on-call person
            on_call = (
                await self.supabase.table("on_call_schedules")
        ...
        team = await self.supabase.table("teams").select("owner_id").eq("id", team_id).execute()

        return team.data[0]["owner_id"] if team.data else None

    async def reassign_incident(
        self, incident_id: str, new_assignee: str, reassigned_by: str, reason: str = "manual"
    ) -> dict:
        """
        ...
        current = (
            await self.supabase.table("incident_assignments")
            .select("*")
        ...
        if current.data:
            ...
            await (
                self.supabase.table("incident_assignments")
                .update(
                    {
                        "status": "reassigned",
        ...
        return await self.assign_incident(
            incident_id, assigned_to=new_assignee, assigned_by=reassigned_by, reason=reason
        )

    async def unassign_incident(self, incident_id: str, unassigned_by: str) -> dict:
        """
        ...
        await (
            self.supabase.table("incidents")
            .update({"assigned_to": None, "status": "open"})
            .eq("id", incident_id)
            .execute()
        )
    ...
    async def get_team_workload(self, team_id: str) -> list[dict]:
        ...
        # Get team members
        members = (
            await self.supabase.table("team_members")
    ...
    async def get_user_assignments(
        self, user_id: str, status: str = "assigned", limit: int = 20
    ) -> list[dict]:
        """
        ...
        query = (
            self.supabase.table("incidents")
            .select("id, severity, service_id, service_name, created_at, status")
    ...
    async def get_assignment_history(self, incident_id: str) -> list[dict]:
        """
    ...
    async def _log_assignment(self, incident_id: str, assignment: dict):
        """Log assignment to incident timeline"""
        await (
            self.supabase.table("incident_timeline")
            .insert(
                {
                    "incident_id": incident_id,
                    "event_type": "assigned",
    ...
    async def get_team_stats(self, team_id: str) -> dict:
        ...
        incidents = (
            await self.supabase.table("incidents")
            .select("id, severity, assigned_to, status, created_at, resolved_at")
            .eq("team_id", team_id)
            .execute()
        )
        ...
        avg_assignment_time = (
            sum(assignment_times) / len(assignment_times) if assignment_times else 0
        )
