...
class IncidentMemory:
    ...
    async def find_similar_incidents(
        self, incident: dict, limit: int = 5, include_low_confidence: bool = False
    ) -> dict:
        """
        ...
        high_confidence = [s for s in similar if s.get("similarity_score", 0) >= threshold]
    ...
    async def record_feedback(
        self, incident_id: str, suggested_incident_id: str, helpful: bool, user_id: str
    ):
        """
        ...
        """
        await (
            self.supabase.table("suggestion_feedback")
            .upsert(
                {
