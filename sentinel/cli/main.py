...
@click.option('--failure-threshold', default=3, type=click.IntRange(1, 20), show_default=True, help='Failures required before opening an incident')
@click.option('--recovery-threshold', default=2, type=click.IntRange(1, 20), show_default=True, help='Healthy checks required before resolving an incident')
@click.pass_context
            ...
            while True:
                ...
                try:
                    ...
                    if is_healthy:
                        ...
                        if active_incident_id and db.connected and should_resolve_incident(
                            state,
                            thresholds,
                        ...
                        if service and user and db.connected and should_open_incident(
                            state,
                            thresholds,
...
@click.option('--status', type=click.Choice(['detecting', 'alerting', 'investigating', 'resolved']), help='Filter by status')
@click.pass_context
def incidents_list(ctx, limit, severity, status):
    ...
    for inc in incidents_data:
        ...
        sev_color = {'critical': 'red', 'high': 'yellow', 'medium': 'blue', 'low': 'white'}.get(sev_raw, 'white')
