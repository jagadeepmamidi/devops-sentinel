...
def services_list(project, output_json):
    ...
    for svc in services_data:
        ...
        latency = f"{svc.get('last_response_time_ms', 0)}ms"

        status_color = {"healthy": "green", "degraded": "yellow", "down": "red"}.get(
            status, "white"
        )
...
def services_check(service_id):
    ...
    if not isinstance(url, str) or not url:
        click.echo(click.style("Error: Service URL is missing.", fg="red"))
        return
    ...
    async def _check():
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            try:
                ...
                elapsed = int(
                    (datetime.now(timezone.utc).replace(tzinfo=None) - start).total_seconds() * 1000
                )
                is_healthy = response.status_code == 200

                db.log_health_check(service_id, response.status_code, elapsed, is_healthy)
                db.update_service_status(
                    service_id, "healthy" if is_healthy else "degraded", elapsed
                )

                status = (
                    click.style("OK HEALTHY", fg="green")
                    if is_healthy
                    else click.style("WARN DEGRADED", fg="yellow")
                )
                ...
                error_text = f"{e!s}"[:50]
                click.echo(f"  {click.style('X DOWN', fg='red')} | {error_text}")
