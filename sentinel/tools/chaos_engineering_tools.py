...
class ChaosEngineeringTools:
    ...
    async def inject_latency(
        self, service_id: str, latency_ms: int, duration_seconds: int, probability: float = 1.0
    ) -> dict:
        """
        ...
        try:
            experiment_id = (
                f"chaos-latency-{service_id}-{int(datetime.now(timezone.utc).timestamp())}"
            )
        except (OverflowError, OSError, ValueError):
            experiment_id = f"chaos-latency-{service_id}-0"
        ...
        await self._log_experiment(
            ...
            service_id=service_id,
            experiment_type="latency_injection",
            config={"latency_ms": latency_ms, "duration_seconds": duration_seconds},
            status="completed",
            results={"baseline": baseline, "degraded": degraded, "impact": impact},
        )
    ...
    async def kill_service(self, service_id: str, duration_seconds: int) -> dict:
        """
        ...
        """
        try:
            experiment_id = f"chaos-kill-{service_id}-{int(datetime.now(timezone.utc).timestamp())}"
        except (OverflowError, OSError, ValueError):
            experiment_id = f"chaos-kill-{service_id}-0"
    ...
    async def simulate_traffic_spike(
        self, service_id: str, multiplier: float, duration_seconds: int
    ) -> dict:
        """
        ...
        try:
            experiment_id = (
                f"chaos-traffic-{service_id}-{int(datetime.now(timezone.utc).timestamp())}"
            )
        except (OverflowError, OSError, ValueError):
            experiment_id = f"chaos-traffic-{service_id}-0"
        ...
        await self._log_experiment(
            ...
            config={"multiplier": multiplier, "duration_seconds": duration_seconds},
            status="running",
        )
    ...
    async def test_dependency_failure(
        self, service_id: str, dependency_id: str, failure_mode: str = "timeout"
    ) -> dict:
        """
        ...
        """
        try:
            experiment_id = f"chaos-dep-{service_id}-{int(datetime.now(timezone.utc).timestamp())}"
        except (OverflowError, OSError, ValueError):
            experiment_id = f"chaos-dep-{service_id}-0"
        ...
        await self._log_experiment(
            ...
            config={"dependency_id": dependency_id, "failure_mode": failure_mode},
            status="completed",
            results={
    ...
    async def _measure_service_health(self, service_id: str) -> dict:
        ...
        # Get service details
        service = await self.supabase.table("services").select("*").eq("id", service_id).execute()
    ...
    async def _get_dependent_services(self, service_id: str) -> list[str]:
        ...
        deps = (
            await self.supabase.table("service_dependencies")
            .select("source_service_id")
            .eq("target_service_id", service_id)
            .execute()
        )

        return [d["source_service_id"] for d in deps.data] if deps.data else []

    async def _monitor_cascade_failures(self, services: list[str], duration: int) -> list[dict]:
    ...
    def _calculate_impact(self, baseline: dict, degraded: dict) -> dict:
        ...
        error_rate_increase = degraded.get("error_rate", 0) - baseline.get("error_rate", 0)
    ...
    async def _log_experiment(
        ...
        await (
            self.supabase.table("chaos_experiments")
            .insert(
                {
                    "id": experiment_id,
                    "service_id": service_id,
...
if __name__ == "__main__":
    ...
    async def test_chaos():
        ...
        result = await chaos.inject_latency(service_id="svc-1", latency_ms=500, duration_seconds=5)
        print(f"Latency test: {result}")

        # Test service kill
        result = await chaos.kill_service(service_id="svc-1", duration_seconds=10)
