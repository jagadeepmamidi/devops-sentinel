...
export default function OperatorIncidents() {
  ...
  useEffect(() => {
    ...
    operatorFetch('/api/incidents', token)
      .then((payload) => {
        if (!cancelled) {
      ...
      <main id="operator-main" className="site-main site-container operator-main">
        <div className="operator-stack">
          <section className="site-card operator-panel">
            ...
            <p className="site-text">Browse incident state, severity, and generated postmortems.</p>
            ...
            <form className="operator-token-form" onSubmit={handleSaveToken}>
              ...
              <button className="site-btn secondary" type="button" onClick={() => { saveOperatorToken(''); setTokenInput(''); setIncidents([]) }}>
                Clear
              </button>
            </form>
          ...
          <section className="site-card operator-panel">
            ...
            {!loading && !token ? <p className="operator-empty">Save a token to load incidents.</p> : null}
              ...
              <table className="operator-table">
                ...
                <tbody>
                  {incidents.map((incident) => (
                    <tr key={incident.id}>
                      <td>
                        <Link className="operator-link" to={`/operator/incidents/${incident.id}`}>
                          {incident.service_name || incident.service_id}
                        </Link>
                        <div className="site-text">{incident.error_message || 'No error summary recorded.'}</div>
                      </td>
                      <td>
                        <span className={`operator-status ${incident.status || 'detecting'}`}>{incident.status}</span>
                      </td>
