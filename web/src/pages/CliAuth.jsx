...
export default function CliAuth() {
	...
	const supabaseUrl =
		searchParams.get("supabase_url") || import.meta.env.VITE_SUPABASE_URL || "";
	const redirectUri =
		searchParams.get("redirect_uri") || "http://localhost:54321/callback";
			...
			<main id="auth-main" className="site-main site-container cli-auth-main">
				<section className="site-card cli-auth-panel">
					...
					<p className="site-text">
						Authenticate in browser, then return to your local CLI callback so
						terminal session can continue automatically.
					</p>

					{flow === "device" && (
						<p className="site-text cli-auth-note">
							Device flow detected. After login, copy the redirected callback
							URL and paste it in the terminal prompt.
						</p>
					)}
					...
					{!hasConfig && (
						<div className="cli-auth-warning">
							Missing auth context. Open this page from{" "}
							<code className="site-inline-code">sentinel login</code> so it
							includes <code className="site-inline-code">supabase_url</code>{" "}
							and <code className="site-inline-code">redirect_uri</code>.
						</div>
					)}
				</section>
